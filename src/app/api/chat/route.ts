...
// ── 安全清理 JSON 字串的工具函式 (超級強化版) ───────────
function cleanJsonResponse(text: string): string {
  if (!text) return "";
  let str = String(text);
  
  // 直接尋找字串中的第一個 '{' 和最後一個 '}'，忽略 AI 的碎碎念
...
「因為這是你第一次記錄比較豐盛的餐點，我幫你準備了一顆專屬的『代謝蛋』🥚！接下來，只要我們一起完成右側的代謝任務，這這次的蛋就會慢慢孵化喔！我們一起努力吧！」
`;
      }
...
※ 注意：tasks 陣列的長度必須等於 taskCount，若為健康飲食不需任務，請回傳空陣列 []。`;
      ...
      // 🌟 修復 1：終極防呆！只要 AI 有發派代謝任務，這餐絕對是需要還債的 Cheat Meal (不健康)！
      if (parsedTasks.length > 0 || parsedTaskCount > 0) {
        extraHeaders["X-Meal-Type"] = "cheat";
      } else {
        extraHeaders["X-Meal-Type"] = parsedHealthLevel >= 3 ? "cheat" : "healthy";
      }

      // 🌟 修復 2：加上這行，確保前端在各種環境下都能完美讀取到 Header 裡的任務資料
      ...
      const encoder = new TextEncoder();
      // ... 下面接 const stream = new ReadableStream({ ...
...
【重要指示：使用真實資料】
請務必使用你的 Google 搜尋能力，查詢該餐廳（例如：WakuWaku Burger）最新的真實菜單與品項。
絕對不可自行捏造不存在的餐點！
請從真實菜單中，推薦4~6個考慮營養均衡的餐點。

【輸出格式要求】
請嚴格遵守專業諮詢師語氣，不帶道德綁架或裝可愛，只輸出 JSON：
...
【輸出格式要求】
請嚴格遵守專業諮詢師語氣，不帶道德綁架或裝可愛，只輸出 JSON：
{"intro":"客觀同理其生理需求（如氣溫、解渴），不使用過度渲染情緒的字眼","shops":[{"name":"店名","mapsUrl":"連結","isOpen":true,"walkingMinutes":5,"items":[{"name":"品項","size":"M","sugar":"無糖","ice":"少冰","calories":100,"price":55}]}],"healthy_tip":"從維持代謝平衡角度給予中立客觀的專業建議，不使用「喔」、「耶」結尾"}
對話：${historyText || "（無）"}`;
      const text = await generateTextWithFallback(prompt);
